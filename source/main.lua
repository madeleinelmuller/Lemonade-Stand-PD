...
index 0000000000000000000000000000000000000000..44d0178ec7a2ff698177dc3075d50fc1101ea7a9
--- /dev/null
+++ b/source/main.lua
@@ -0,0 +1,299 @@
...
+    local y = 36
+    local function meter(label, value, max)
+        local width = 160
+        gfx.drawText(label, 12, y)
+        gfx.drawRoundRect(10, y + 12, width, 10, 3)
+        gfx.fillRoundRect(10, y + 12, width * clamp(value / max, 0, 1), 10, 3)
+        y += 28
+    end
...
+        local value
+        if action.id == "price" then value = string.format("$%.2f", world.price) end
+        if action.id == "recipe" then value = string.format("%.0f%% tart", world.recipe * 100) end
+        if action.id == "marketing" then value = string.format("$%d", math.floor(world.marketing)) end
+        if action.id == "cups" then value = string.format("%d (%.2f ea)", world.inventory.cups, costs.cups) end
+        if action.id == "lemons" then value = string.format("%d (%.2f ea)", world.inventory.lemons, costs.lemons) end
+        if action.id == "sugar" then value = string.format("%d (%.2f ea)", world.inventory.sugar, costs.sugar) end
+
+        if isSelected then
...
+    if uiState.phase == "selling" then hint = "Selling..." end
+    if uiState.phase == "results" then hint = "A:Next day  B:Adjust" end
...
+            world.cash -= cost
+            world.inventory[resource] += delta
...
+        world.inventory[resource] -= actual
+        world.cash += refund
...
+
+    world.inventory.cups -= customers
+    world.inventory.lemons -= customers
+    world.inventory.sugar -= customers
+    world.cash += profit
+
